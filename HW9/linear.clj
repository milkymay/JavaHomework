...
;(or (number? (first args))
;(every? #(similarCounts % (first args)) args)


(defn checkTensor [arg]
      (if (every? number? arg)
        true
        (if (every? #(vector? %))
          (if (every? #(similarCounts % (first arg)))
            (every? checkTensor (apply mapv vector arg))
            false)))
      )

(defn applyTensorOperation [f & args]
      {:pre [(checkTensor (first args))]}
      (if (number? (first args))
        (apply f args)
        (apply mapv (partial applyTensorOperation f) args))
      )



(defn t+ [& args] (apply (partial applyTensorOperation +) args))
(defn t- [& args] (apply (partial applyTensorOperation -) args))
(defn t* [& args] (apply (partial applyTensorOperation *) args))